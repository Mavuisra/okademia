...
from .offline import OfflineMixin, OfflineSynchronizer
from .management.commands.backup_database import Command as BackupCommand
from .management.commands.sync_offline_data import Command as SyncCommand
...
class BackupCommandTestCase(TestCase):
    ...
    @patch("django.db.connection")
    def test_postgres_backup(self, mock_connection, mock_subprocess_run):
        ...
        # Create the backup directory (needed for the test to pass)
        backup_dir = os.path.join(self.temp_dir, "backups")
        ...
        # Check that subprocess.run was called for pg_dump
...
class OfflineViewsTestCase(TestCase):
    ...
    def test_synchronize_endpoint(self, mock_process_queue):
        ...
        response = self.client.post(
            reverse("belletin:synchronize_offline_data"),
            {},
    ...
    def test_queue_status_view(self):
        ...
        response = self.client.get(reverse("belletin:offline_queue_status"))
